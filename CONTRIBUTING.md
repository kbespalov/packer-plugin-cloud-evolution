...
4. Live API quirks go in the README table and in comments next to the request.
   Fail fast on `error`/`failed` resource states; do not wait out `state_timeout`.
   A 404 on Find/Get is "missing"; any other error is fatal (never "name is free").
